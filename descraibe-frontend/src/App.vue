  ...
  <header>
    <div class="menu">
      ...
      <div class="menu-buttons">
        ...
        <span v-html="devModeResetButton"></span>
      </div>
    </div>
  </header>
  <main class="main">
...
export default {
  ...
  methods: {
    ...
    initializeGame() {
      ...
      // when run from "npm run dev" then "curEnv"==="development" 
      // when run from "npm run build" then "curEnv"==="production"  
      const curEnv = import.meta.env.MODE;
      // This will display the trash can when running in dev mode
      if (curEnv==='development') {
        this.devModeResetButton=`<button type="button" @click="clearStorage()" class="menu-button">
